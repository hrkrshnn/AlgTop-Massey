...
*** 3.2
    ...
    infinitely many $\Z_n$, which is clearly an infinite group.)
...
*** TODO 4.1
    ...
    equal, since $x_1x_2x_1x_2 \neq 1$. These elements are of infinite
    order. To show that the center contains only identity, given an
    arbitrary element $y$, let $y_1 \in G_{y_1}$ denote the first
    element of the word $y$ and $y_2$ denote an element other than
    unity in and not contained in $G_{y_1}$. Clearly $yy_2 \neq y_2y$.

    (TODO: Need to improve the reasons.)
** 5
*** 5.1
